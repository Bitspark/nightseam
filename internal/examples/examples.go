...
// Builder reuses a family's validation scopes for deterministic witnesses.
type Builder struct {
	f          *render.Family
...
// New constructs the shared synthesis context for one rendered declaration.
func New(f *render.Family) *Builder {
	b := &Builder{f: f, families: map[string]*render.Family{}, schemas: map[string]*runtime.Schema{}}
...
func (b *Builder) context(scope []model.Parameter, uses []render.Use) (*exampler, ExampleInfo, *runtime.Schema) {
	x := newExampler(b.f)
	info := ExampleInfo{}
