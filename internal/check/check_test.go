...
// probe is a family with a protocol; codex a second one; a test's family is
...
func world(family string, files map[string]string) *analysis.Family {
	families := map[string]map[string]string{
		...
		"carrier": {
			...
			"protocol.json": modeltest.Protocol(`"imports": ["probe"], "parameters": [{"name": "S", "of": "protocol"}], "types": {"Frame": {"kind": "record", "fields": [{"name": "message", "type": "S.Envelope"}]}, "Plain": {"kind": "record", "fields": []}}, "server": {"methods": {"relay": {"request": "Frame", "result": "probe.Envelope"}}}`),
		},
	}
...
func TestRules(t *testing.T) {
	...
	for name, c := range map[string]struct {
		...
		"a parameter in the model tier":                   {map[string]string{"model.json": m(`"A": {"kind": "record", "fields": [{"name": "m", "type": "S.Envelope"}]}`), "protocol.json": p(`"parameters": [{"name": "S", "of": "protocol"}], "types": {"B": {"kind": "record", "fields": [{"name": "m", "type": "S.Envelope"}]}}`)}, "tier_violation@model.json#/types/A/fields/0/type"},
		...
		"a layer's namespace":                             {map[string]string{"model.json": m(`"E": {"kind": "enum", "values": ["v"]}`), "protocol.json": p(`"server": {"methods": {"auth.login": {"result": "string"}, "live.peek": {"result": "string"}}, "events": {"channel.opened": {"type": "string"}, "identity.seen": {"type": "string"}, "mine.own": {"type": "string"}}}`)}, "reserved_name@protocol.json#/server/events/channel.opened reserved_name@protocol.json#/server/events/identity.seen reserved_name@protocol.json#/server/methods/auth.login reserved_name@protocol.json#/server/methods/live.peek"},
