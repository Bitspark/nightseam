...
export function parseRoute(hash) {
  ...
  try { return hash.slice(2).split("/").map(decodeURIComponent); } catch { return null; }
}
...
export function typeText(expr) {
  ...
  if (expr.apply) return `${expr.apply}<${Object.entries(expr.with ?? {}).map(([k, v]) => `${k} = ${typeText(v)}`).join(", ")}>`;
...
export function buildAtlas(checkout) {
  ...
  const atlas = { families, types: new Map(families.map((f) => [f.Name, new Map(allTypes(f).map((t) => [t.Name, t]))])), shared: [], languages: [] };
  ...
  for (const f of families) {
    ...
    for (const side of [f.Server, f.Client]) {
      for (const op of [...list(side?.Methods), ...list(side?.Events)]) Object.keys(op.Languages ?? {}).forEach((l) => languages.add(l));
    }
  }
...
export function familyView(atlas, name, lens = "wire") {
  ...
  if (!f) return null;
  const exchanges = [];
  for (const [sideName, side] of [["server", f.Server], ["client", f.Client]]) {
    for (const [kind, ops] of [["method", side?.Methods], ["event", side?.Events]]) {
      for (const op of list(ops)) {
        const initiator = sideName === "server" ? "client" : "server";
        // A side declares what it receives: its methods are called and its
        // events are emitted by the opposite peer.
        const path = [name, kind, sideName, op.Name];
        exchanges.push({ id: anchor(...path), href: route(...path), name: op.Name,
          display: op.Languages?.[lens]?.Name || op.Name, kind, side: sideName, initiator,
          description: op.Description ?? "", op, frames: kind === "method" ? op.Frames : { Event: op.Frame },
  ...
  const types = allTypes(f).map((t) => ({ ...t, display: t.Languages?.[lens]?.Name || t.Name, id: anchor(name, "type", t.Name), href: route(name, "type", t.Name) }));
  const errors = list(f.Errors).map((e) => ({ ...e, id: anchor(name, "error", e.Code), href: route(name, "error", e.Code),
    also: atlas.families.filter((other) => other.Name !== name && list(other.Errors).some((x) => x.Code === e.Code)).map((f) => f.Name) }));
  return { family: f, id: anchor(name), exchanges, types, errors,
...
export function finder(atlas, query = "") {
  ...
  for (const f of atlas.families) {
    ...
    entries.push({ name: f.Name, family: f.Name, kind: "family", id: v.id, href: route(f.Name), description: f.Source });
    for (const x of v.exchanges) entries.push({ ...x, family: f.Name });
    for (const t of v.types) entries.push({ name: t.Name, family: f.Name, kind: "type", id: t.id, href: t.href, description: t.Description });
    for (const e of v.errors) entries.push({ name: e.Code, family: f.Name, kind: "error", id: e.id, href: e.href, description: e.Description });
  }
  const words = query.toLowerCase().trim().split(/\s+/);
  return entries.filter((e) => words.every((w) => `${e.family} ${e.name} ${e.kind} ${e.description ?? ""}`.toLowerCase().includes(w)));
}
...
export function annotate(atlas, family, expression, value, options = {}) {
  ...
  const child = (expr, value, extra = {}) => annotate(atlas, r.family, expr, value, { trail, bindings: r.bindings, ...extra });
  if (r.type) {
    const t = r.type;
    row.kind = t.Kind;
    row.href = route(r.family, "type", t.Name);
    const identity = `${r.family}.${t.Name}`;
    if (trail.includes(identity)) { row.recursive = true; return row; }
    const next = [...trail, identity];
    if (t.Kind === "alias") {
      const target = child(t.Alias, value, { trail: next });
      return { ...target, ...row, kind: target.kind, children: target.children, variants: target.variants, recursive: target.recursive };
    }
    if (t.Kind === "record" || t.Kind === "entity") {
      row.children = list(t.Fields).map((field) => child(field.Declared ?? field.Type, value?.[field.Name], {
        trail: next, name: field.Name, presence: field.Required ? "required" : "optional", description: field.Description ?? "",
      }));
      if (t.Open) row.children.push({ name: "…", type: "json", ghost: true, presence: "optional", description: "Additional members are preserved.", children: [] });
    }
    if (t.Kind === "union") {
      row.variants = list(t.Variants).map((v) => {
        ...
        const children = [tag];
        if (!v.Empty) children.push(child(v.Declared ?? v.Type, example?.[t.Value], { trail: next, name: t.Value, presence: "required" }));
  ...
  if (expr?.nullable) {
    ...
    return { ...target, ...row, kind: target.kind, children: target.children, variants: target.variants, recursive: target.recursive, href: target.href };
  }
  if (expr?.array) {
    row.kind = "array";
  ...
  if (expr?.map) {
    row.kind = "map";
    row.children = Object.entries(value && typeof value === "object" ? value : {}).map(([name, v]) => child(expr.map, v, { name }));
  }
  return row;
}
...
export function shapeKey(atlas, family, expression) {
  function shape(family, expr, trail = [], bindings = new Map()) {
    ...
    if (t) {
      ...
      if (t.Kind === "enum") return ["enum", t.Values];
      if (t.Kind === "union") return ["union", t.Tag, t.Value, list(t.Variants).map((v) => [v.Tag, v.Empty ? "empty" : child(v.Declared ?? v.Type, next)])];
      return [t.Kind, !!t.Open, t.Key, list(t.Fields).map((f) => [f.Name, !!f.Required, !!f.Nullable, !!f.Unique, f.Min, f.Max, f.Length, f.Pattern, child(f.Declared ?? f.Type, next)])];
    }
    const e = r.expr;
...
const refRoute = (family, ref) => ref.Kind === "type" ? route(family, "type", ref.Name) : route(family, ref.Kind, ref.Side, ref.Name);

export function annotationHTML(row, depth = 0) {
  const type = row.href ? link(row.href, row.type) : esc(row.type);
  const value = row.value && typeof row.value === "object" ? (Array.isArray(row.value) ? `[${row.value.length} items]` : "{…}") : json(row.value);
  ...
  const children = list(row.children).map((c) => annotationHTML(c, depth + 1)).join("");
  const variants = list(row.variants).map((v) => `<div class="variant"><span class="small-label">${esc(v.tag)}</span>${v.children.map((c) => annotationHTML(c, depth + 1)).join("")}</div>`).join("");
...
function exchangeHTML(atlas, f, x, lens) {
  ...
  const governance = [list(session?.Decides).includes(x.name) ? "decides" : "", list(session?.Asks).includes(x.name) ? "asks" : "",
    session?.Conversation?.Event === x.name ? `conversation at ${session.Conversation.Path}` : ""].filter(Boolean);
  const names = [chip(`wire: ${x.name}`), ...Object.entries(op.Languages ?? {}).map(([name, lang]) => chip(`${name}: ${lang.Name}`))].join("");
  const result = x.kind === "method" ? typeText(op.DeclaredResult) : typeText(op.Declared);
  const left = x.initiator === "client" ? `<span class="name client">${esc(x.display)}</span>` : `<span class="response-label">${esc(result)}</span>`;
  const right = x.initiator === "server" ? `<span class="name server">${esc(x.display)}</span>` : `<span class="response-label">${esc(result)}</span>`;
  const frames = x.kind === "event"
    ? frameHTML(atlas, f.Name, `${x.initiator} → ${opposite} · event`, x.initiator, op.Frame, op.Declared, "data")
    : frameHTML(atlas, f.Name, `${x.initiator} → ${opposite} · request`, x.initiator, op.Frames?.Request, op.DeclaredRequest, "params") + frameHTML(atlas, f.Name, `${opposite} → ${x.initiator} · response`, opposite, op.Frames?.Response, op.DeclaredResult, "result");
  const refusals = list(op.Frames?.Refusals).map((r) => frameHTML(atlas, f.Name, `refusal · ${r.Code}`, opposite, r.Frame)).join("");
...
export function familyHTML(atlas, name, lens = "wire") {
  ...
  const hall = v.exchanges.map((x) => link(x.href, "", `class="${x.initiator}" title="${esc(x.name)} · ${x.weight} values" aria-label="${esc(x.name)}"`).replace("></a>", `><i style="height:${12 + Math.round(52 * x.weight / max)}px"></i></a>`)).join("");
  const board = ["client", "server"].map((side) => {
    const exchanges = v.exchanges.filter((x) => x.initiator === side);
    return exchanges.length ? `<div class="group-label ${side}">${side} begins · ${exchanges.length}</div>${exchanges.map((x) => exchangeHTML(atlas, f, x, lens)).join("")}` : "";
  }).join("");
  const vocabulary = v.types.map((t) => `<article class="vocab-row" id="${esc(t.id)}">${link(t.href, t.display)}<div><p>${esc(t.Description)}</p><div class="chips">${chip(t.Kind)}${t.Inline ? chip("derived inline") : ""}${t.Carried ? chip(`carried from ${t.From}`) : ""}${list(t.Parameters).map((p) => chip(`${p.Name}: ${p.Of || "type"}`)).join("")}</div>${list(t.UsedBy).length ? `<div class="references">Used by ${t.UsedBy.map((ref) => link(refRoute(name, ref), `${ref.Name} · ${ref.At}`)).join("")}</div>` : ""}</div></article>`).join("");
  const errors = v.errors.map((e) => `<article class="error-row" id="${esc(e.id)}">${link(e.href, e.Code)}<div><p>${esc(e.Description)}</p><div class="references">${v.exchanges.filter((x) => list(x.op.Errors).includes(e.Code)).map((x) => link(x.href, x.name)).join("")}${e.also.length ? `Also in ${e.also.map((n) => link(route(n, "error", e.Code), n)).join("")}` : ""}</div></div></article>`).join("");
  const shared = atlas.shared.map((s) => `<tr><td>${esc(s.name)}</td><td>${s.members.map((m) => link(route(m.family, "type", s.name), m.family)).join("")}</td><td>${s.same ? "same shape" : "different shapes"}</td></tr>`).join("");
  return `<div id="${esc(v.id)}" class="hero"><div><span class="eyebrow">Protocol atlas / ${esc(f.Builtin ? "built-in family" : "family")}</span><h1>${esc(f.Name)}<span class="client">.</span></h1><p class="lede">${f.Protocol ? "What each side says, what it carries, and the vocabulary they share." : "The shared vocabulary, its shapes and the values they carry."}</p><div class="facts"><span><strong>${count}</strong> exchanges</span><span><strong>${v.types.length}</strong> types</span><span><strong>${v.errors.length}</strong> errors</span></div></div><div><div class="legend"><span class="client"><i class="dot"></i>Client / lamp</span><span class="server"><i class="dot"></i>Server / moon</span></div><div class="hall" aria-label="Exchanges weighted by example values">${hall || '<span class="small-label">A vocabulary without exchanges</span>'}</div><p class="hall-caption">One mark per exchange. Height follows the values it carries.</p></div></div><nav class="section-nav" aria-label="Sections"><a href="#board">01 Exchanges</a><a href="#vocabulary">02 Vocabulary</a><a href="#errors">03 Errors</a><a href="#across">04 Across families</a></nav>${parametersHTML(f.Parameters)}${f.Session?.Conversation ? `<p class="references">Conversation arrives at ${link(route(name, "event", "client", f.Session.Conversation.Event), f.Session.Conversation.Event)} · ${esc(f.Session.Conversation.Path)}</p>` : ""}<section id="board"><div class="section-heading"><span class="section-no">01</span><h2>The exchange board</h2><span class="section-note">Open a row to see its anatomy</span></div><div class="board-head"><span class="client">CLIENT</span><span>ON THE WIRE</span><span class="server">SERVER</span></div>${board || '<p class="empty">This family declares no exchanges.</p>'}</section><section id="vocabulary"><div class="section-heading"><span class="section-no">02</span><h2>A shared vocabulary</h2><span class="section-note">Open a type to explore its shape</span></div>${vocabulary || '<p class="empty">No types declared.</p>'}</section><section id="errors"><div class="section-heading"><span class="section-no">03</span><h2>When an exchange fails</h2></div>${errors || '<p class="empty">No public errors declared.</p>'}</section><section id="across"><div class="section-heading"><span class="section-no">04</span><h2>Across families</h2></div>${shared ? `<table class="comparison"><thead><tr><th>Shared name</th><th>Declared in</th><th>Wire shape</th></tr></thead><tbody>${shared}</tbody></table>` : '<p class="empty">No type name is shared by multiple families.</p>'}</section>`;
}
...
export function typeHTML(atlas, family, name, lens = "wire") {
  ...
  return `<span class="eyebrow">${esc(family)} / ${esc(t.Kind)}</span><h2 id="drawer-title">${esc(language?.Name || t.Name)}</h2><p class="description">${esc(t.Description)}</p><div class="type-meta">${esc(t.Inline ? "Derived from an inline shape. " : "")}${esc(t.Open ? "Open record. " : "")}${t.Key ? `Identity: ${esc(t.Key)}. ` : ""}${list(t.Extends).length ? `Extends ${esc(t.Extends.map(typeText).join(", "))}.` : ""}</div>${parametersHTML(t.Parameters)}${code(json(t.Example))}<div class="annotation">${annotationHTML(annotate(atlas, family, name, t.Example))}</div>${list(t.Values).length ? `<p>Values: ${esc(t.Values.map((v) => JSON.stringify(v)).join(" · "))}</p>` : ""}${list(t.UsedBy).length ? `<h3 class="declaration">Used by</h3><div class="references">${t.UsedBy.map((r) => link(refRoute(family, r), `${r.Name} · ${r.At}`)).join("")}</div>` : ""}${language?.Declare ? `<h3 class="declaration">Declaration · ${esc(lens)}</h3>${code(language.Declare)}` : ""}`;
}
