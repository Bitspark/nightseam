...
function navigate() {
  ...
  if (["#board", "#vocabulary", "#errors", "#across", "#content"].includes(location.hash)) return;
  const parts = parseRoute(location.hash || "#/");
  ...
  document.getElementById("families").innerHTML = atlas.families.map((f) => link(route(f.Name), f.Name, f.Name === name ? 'aria-current="page"' : "")).join("");
  if (parts?.[1] === "type" && view.types.some((t) => t.Name === parts[2])) {
    ...
    document.getElementById("breadcrumbs").innerHTML = link(route(name), name) + " / " + breadcrumbs.map((b) => link(b.href, b.name)).join(" / ");
...
function search() {
  ...
  document.getElementById("results").innerHTML = results.length
    ? results.map((e) => `<a class="search-result" href="${esc(e.href)}"><span>${esc(e.name)}</span><small>${esc(e.family)} · ${esc(e.kind)}</small></a>`).join("")
    : '<p class="empty">No matching names or descriptions.</p>';
}
function openFinder() { search(); searchDialog.showModal(); document.getElementById("search").focus(); }
document.getElementById("find").addEventListener("click", openFinder);
document.getElementById("search").addEventListener("input", search);
document.getElementById("search").addEventListener("keydown", (event) => {
  if (event.key === "ArrowDown") { event.preventDefault(); document.querySelector("#results a")?.focus(); }
...
document.addEventListener("keydown", (event) => {
  if (event.key === "/" && !event.ctrlKey && !event.metaKey && !["INPUT", "TEXTAREA", "SELECT"].includes(event.target.tagName) && !drawer.open && !searchDialog.open) { event.preventDefault(); openFinder(); }
});
document.addEventListener("click", (event) => {
  ...
  if (a.getAttribute("href") === location.hash) { event.preventDefault(); navigate(); }
});
...
lensSelect.addEventListener("change", () => { lens = lensSelect.value; navigate(); });
