...
func TestGoUnionNamesMatchTable(t *testing.T) {
	...
	var table struct {
		GoUnions struct {
			Rows []struct{ Name, Override, Tag, KindType, Field, Constant, Wrapper string }
		}
	}
	...
	for _, row := range table.GoUnions.Rows {
		t.Run(row.Name+"/"+row.Tag+"/"+row.Override, func(t *testing.T) {
			...
			for _, pair := range [][2]string{
				{u.kind, row.KindType}, {u.fields[row.Tag], row.Field}, {u.constants[row.Tag], row.Constant}, {u.wrappers[row.Tag], row.Wrapper},
			} {
				if pair[0] != pair[1] {
...
func TestUnionPlanRejectsCollisions(t *testing.T) {
	for name, c := range map[string]struct {
		...
		"payload wrapper":     {`"Part":{"kind":"union","tag":"kind","variants":{"text":"string"}},"PartTextValue":{"kind":"record","fields":[]}`, "", "generated_name_collision", "model.json#/types/Part/variants/text"},
		"widening helper":     {`"Part":{"kind":"union","tag":"kind","variants":{"text":"string"}},"Rich":{"kind":"union","tag":"kind","extends":["Part"],"variants":{"count":"integer"}},"WidenRichFromPart":{"kind":"record","fields":[]}`, "", "generated_name_collision", "model.json#/types/Rich/extends/0"},
