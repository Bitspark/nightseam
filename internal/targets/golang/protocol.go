...
func emitValidation(f *file) {
	...
	f.linef("var schema = %s.MustSchema(%s, map[string]func(string, []byte) error{%s})", runtime, quote(f.family.Wire), strings.Join(imported, ", "))
	f.line("")
	f.linef("// %s verifies a named contract value, including null and field presence.", identValidateRaw)
	f.linef("func %s(name string, data []byte) error { return schema.%s(name, data) }", identValidateRaw, identValidateRaw)
