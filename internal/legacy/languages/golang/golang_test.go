...
func TestRenderPlacesAndFormats(t *testing.T) {
	...
	files, err := New(Options{Module: "example.test/generated"}).Render(api)
	if err != nil {
		t.Fatal(err)
	}
	var paths []string
	...
	if !strings.Contains(string(files[2].Data), `"example.test/generated/api/go/example-protocol"`) {
		t.Fatal("binding does not import the protocol package at the module")
	}
	for _, options := range []Options{{}, {Module: "/abs"}, {Module: "example.test/generated", Runtime: "/abs"}, {Module: "example.test/generated", ClientPath: "../outside"}, {Module: "example.test/generated", ProtocolPath: "api/go/x", BindingPath: "api/go/x"}, {Module: "example.test/generated", ClientPath: "api/go/x/"}, {Module: "example.test/generated", BindingPath: "api/go/x:y"}} {
