...
func TestTargetAnswersForBothUnits(t *testing.T) {
	target := Target(&roster{Layout{Family: "api/names/{family}", Checkout: []string{"api/names/index.txt", "api/all/every.txt"}}}, nil)
...
func TestTargetOfOneUnitOwnsNothingOfTheOther(t *testing.T) {
	whole := Target(&roster{Layout{Checkout: []string{"api/all/every.txt"}}}, nil)
	...
	each := Target(&roster{Layout{Family: "api/names/{family}"}}, nil)
