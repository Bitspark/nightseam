...
[#202](https://github.com/Bitspark/nightseam/issues/202) does. What is not
...
The four identities the live layer must keep apart are named where they
...
response. Sharing a representation makes none of them another.
...
| a live binding — export and import | composition, pending #202 | composition argued in #202 and since **run**: one channel per exported binding, a peer served on it, the handle passed as data with its contract, in both languages across a real socket ([what a consumer composes](runtime/compositions.md)). The open question — whether repeated import shares one dispatch attachment through the tunnel's public surface — is answered yes: `Tunnel.Channel` returns the one channel and an importer that keeps a table gets one attachment, so the tunnel needs neither repair nor exposure. What does not hold is the *generated* `Open`, which attaches per call and checks no contract; both are repairs it owes, not guarantees the channel cannot give. The construction named no such guarantee. Its shipping, and the lifetime rules, wait on #202's verdict. |
| scope — identity bounded by one connection | composition | a handle is already meaningless outside its carrying connection; reconnection is a new scope. Durable references that survive it are a consumer's persistence — domain semantics, not admitted ([#202](https://github.com/Bitspark/nightseam/issues/202) option b). |
| release, and aliases | composition, pending #202 | a channel's close, idempotent, reaching every alias; separate lifetimes are separate bindings. It does not invoke the consumer's `cancel`. |
| forwarding into another scope | composition, pending #202 and #205 | an imported proxy exported again; releasing the forwarding binding does not release the origin, and the origin's closure reaches the proxy as a failure. |
| the registry — the live runtime | composition, shipped | bookkeeping of bindings and proxies over the rows above, generated conversion at the boundary; shipped because every consumer would write it identically. It is not a catalog, an entity store or a durable host. |
| stream, cell, topic | compositions | derived protocols over callables: receive/close with stated completion, ordering and cancellation; get/set/subscribe with stated consistency; emit/subscribe with stated retention and fan-out. A returned callable alone implies none of those rules. Demonstrated by #205; none becomes a kind. |
