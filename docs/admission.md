...
| concept | class | the argument |
| --- | --- | --- |
| declaration identity — `(path, digest)` | primitive exposure, RPC and live | the generated description supplies one revision identity, checked before channel admission or live import can dispatch consumer code. A consumer's `describe()` call checks only after accepting the channel and running a handler, and a model-agnostic intermediary cannot supply that call for every transferred reference. The exposure carries the generated digest beside the existing name and refuses differing specified digests with `contract_mismatch`; absence makes no revision claim. Strict equality is [#292's verdict](https://github.com/Bitspark/nightseam/issues/292#issuecomment-5753285816). Compatibility between distinct revisions remains consumer policy, and declaration identity establishes neither authenticated peer identity nor permission. |
