...
| concept | class | the argument |
| --- | --- | --- |
| Wire carrying profile frames | primitive, RPC surface | an exposure of existing correlation, cancellation, refusal and bounds, not a second request stack. A return capability and received context remain local; the physical peer continues to own its ids and the four frame kinds. Send access is the whole of it: [Bitwire's contract](https://github.com/Bitspark/bitwire) is adopted at v0.2.0 and Nightseam defines no second copy. |
| Endpoint — one owning attachment and closure | primitive, RPC surface | a carrier owes somewhere for its deliveries to go and a way to end; it owes nothing about which handler they reach. Composition attempted and kept: registration, exact and prefix matching, overlap precedence and duplicate-path refusal all state their policy above the primitive, so a generated dispatcher, a dynamic map lookup and a forwarder need not share a registration abstraction ([the decision](decisions/an-invocation-is-a-wire-and-routing-is-composed-above-it.md)). |
| the dispatcher — registration, precedence, selected receiving views | composition, shipped | one owner of one attachment, with exact-before-longest-prefix and a refusal for a duplicate path; sibling and nested views are views of that owner. Shipped because every consumer would write it, identically, and it names no concept of theirs. |
| the invocation lifecycle — capture, execution, control, retirement | composition, shipped | ordinary events of the profile at the return capability's own origin, under the `invocation.` prefix ([vocabulary](wire/vocabulary.md#a-layers-own-vocabulary)) — the same shape as the tunnel's `channel.`, so no primitive is added and no second native interface is owed. Composition attempted and refused only for its alternative: inferring completion from a callback returning, a timeout or a weak map cannot distinguish a replied execution from a running one, which is [Bitwire #20](https://github.com/Bitspark/bitwire/issues/20)'s obstruction. Two independent integrations and an opaque wrapper hold the boundary public. |
...
| forwarding a Wire | composition, shipped | take each endpoint's one attachment and pass messages through the other's Send. Existing roots own admission and correlation. Crossing live scopes still uses generated value converters; raw forwarding does not rewrite hidden references. |
