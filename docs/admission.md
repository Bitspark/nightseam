...
| a live binding — export and import | unresolved candidate | composition argued in #202: one channel per exported binding, a peer served on it, the handle passed as data with its contract. Open: whether repeated import of one binding shares one dispatch attachment through the tunnel's public surface, or needs the tunnel's attach repaired or exposed. Either outcome is a repair or an exposure of an existing primitive, not a new wire kind, unless #205's construction names a guarantee the channel cannot give. Owed by #202's verdict and #205. |
