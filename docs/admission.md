...
| forwarding into another scope | composition, shipped | `Forward(destination, contract, origin)` gives another scope a binding of its own over a function this one imported — an export whose implementation is an import, which a consumer could write itself. It ships spelled out only because the lifetime relationship has to be **stated**: forwarding takes no ownership, releasing the forwarded binding leaves the origin as it was, and an invocation through a released origin fails with the origin's refusal, which is what the destination's caller is told. Since a reference cannot be carried out of band, forwarding is the only way one moves between connections. |
| the scope's bookkeeping — what #200 called the registry | composition, shipped | it did not ship under that name, and the name mattered: a `Scope` with `Export`, `Decode`, `Import`, `Release`, `Forward`, `Counts` and its bounds is bookkeeping of the rows above over one peer, with the conversion generated at the boundary — a codec decodes each reference position as the payload arrives, imports it, and hands the handler a native function. Shipped because every consumer would write it identically. It is not a catalog, an entity store or a durable host, and a `Reference` having no public constructor is what keeps it from becoming one. |
| stream, cell, topic | compositions, demonstrated | derived protocols over callables, each built and run in [what a consumer composes](runtime/compositions.md) with the rules a returned callable does not imply written out: the stream's one report in flight and its exactly one `end`; the cell's versions in order under one writer; the topic's one sequence with a bounded queue per subscriber. The cell awaits each watcher and the topic drops one that falls behind — opposite answers to backpressure, each held by a named test, and neither a rule of the layer's. A returned callable is not a stream: `Job.cancel()` is one callable of one record. None becomes a kind. |
