...
![The projection stages and their interfaces: the consumer's declaration and a program per language at the top; beneath, a Go and a TypeScript realization, each a stack of the family in the language, the sides' adapters, the shared Bitwire contract, the wire realizations and the dispatcher, the layers over the peer, the peer and the seam, meeting at the frames of the profile; every row tagged consumer, generated, library or agnostic](architecture.svg)
...
| layers over the peer | library | compositions a consumer could write and every consumer would write the same way: live bindings in a scope, and channels multiplexed over one peer; and, as modules of their own over the same runtime, the authority profile and the OpenTelemetry adapter | `live.Over` → `Scope`, `Owner`; `tunnel.Tunnel`; `auth/go`, `otel/go` | `liveOver` → `LiveScope`, `LiveOwner`; `Tunnel`; `@nightseam/auth`, `@nightseam/otel` | [the live layer](runtime/live.md), [the tunnel](runtime/tunnel.md), [the auth adapters](auth/adapters.md), [the observer](runtime/observer.md), [admission](admission.md) |
| peer | library | the profile's primitives: correlated requests, events, cancellation, coded refusals, bounds, the `meta` header, trace context, and one observer told at the write | `runtime.Peer` | `DuplexPeer` | [the peer](runtime/peer.md), [the profile](wire/profile.md), [the observer](runtime/observer.md) |
| seam | library | a connection that sends, receives and closes frames — a WebSocket, a pipe, a tunnel channel; the floor, not Nightseam's to decompose | `duplex.Conn` | `FrameConnection` | [the profile](wire/profile.md#the-connection-beneath) |
...
realization's, and each row is tagged by who provides it: the **consumer**
writes the declaration and a program per language — a model of its side as
its own functions, and the composition root that assembles transport, peer,
layers and adapters; the generator **renders** the family in the language and
the sides' adapters into the consumer's tree; Nightseam's **library**, written
once per language, is everything from the wire realizations down — `duplex`,
`runtime`, `live`, `tunnel`, with `auth` and `otel` as modules of their own;
and the three **agnostic** definitions have no language, the contract being
Bitwire's and its per-language packages Bitwire's too. Between the two columns the figure names what each stage
...
- **What is Nightseam's and what is the consumer's.** The top band is the
  consumer's — the declaration is its family, and its program implements
  a model of its side and composes the rest — the family in the language
