...
| `session.cursor` | `{"sequence": N}` | the one attachment a frame was just delivered to, or whose replay just ended | straight after each frame it is delivered, replay and live alike, in the same order |
