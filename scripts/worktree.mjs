...
if (mode === "add") {
  ...
  if (existsSync(path)) {
    ...
    if (registered(path)) { console.error(`${path} is a worktree in use; rm it first, or choose another branch`); process.exit(1); }
  ...
  if (!branch || !valid.test(branch)) { console.error("usage: node scripts/worktree.mjs rm <branch>   (lower case, digits, . _ -)"); process.exit(2); }
  const path = join(dir, branch);
  const had = existsSync(path);
  const known = registered(path);
  if (had) {
    if (known) git("worktree", "remove", "--force", path);
    else {
      // A shell can leave an empty directory after Git unregisters it.
      // A nonempty unregistered directory may still hold somebody's work.
      try { rmdirSync(path); }
      catch (error) { console.error(`cannot remove leftover directory ${path}: ${error.message}`); process.exit(1); }
    }
  }
  git("worktree", "prune");
  let branchGone = false;
  try { git("branch", "-D", branch); branchGone = true; } catch { /* already gone with the PR */ }
  console.log(`${had ? (known ? "removed worktree" : "removed leftover directory") : "no worktree at"} .worktrees/${branch}; local branch ${branchGone ? "deleted" : "already gone"}`);
} else if (mode === "ls") {
