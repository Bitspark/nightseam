...
const serve = async (line: string): Promise<string> => {
  ...
  try {
    ...
    if (error instanceof Failure || error instanceof ProofFailure || error instanceof LiveFailure) return JSON.stringify({ id, error: {code:error.code,message:error.message} });
