...
test('every concrete document example validates with its visible bindings', () => {
  const table = JSON.parse(
    ...
    name: string;
    schemas: Record<string, WireFamily>;
    rows: {
      family: string;
      path: string;
  ...
  let proof = 0;
  for (const corpus of table) {
    const imported: Record<string, Validator> = {};
    for (const [name, wire] of Object.entries(corpus.schemas)) imported[name] = createValidator(wire, imported);
    for (const row of corpus.rows) {
      const where = `${corpus.name}/${row.family}/${row.path}`;
      if (row.unavailable) {
