...
test('a request and an event made inside a handler are children of the handler, and the response repeats it', async (t) => {
  ...
  for (const frame of [
    socket.sent.find((f) => f.method === 'inner'),
    socket.sent.find((f) => f.event === 'progress'),
  ]) {
  ...
  assert.notEqual(
    socket.sent.find((f) => f.method === 'inner')?.traceparent,
    socket.sent.find((f) => f.event === 'progress')?.traceparent,
  );
...
test('a custom propagator sees extract and inject, and what it mints travels verbatim', async (t) => {
  ...
  assert.deepEqual(
    socket.sent.find((frame) => frame.method === 'inner'),
    {
      version: 1,
      kind: 'request',
      id: 'c:1',
