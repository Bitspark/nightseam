...
func ToWire[SJob runtime.Of[STag], SProgress runtime.Of[STag], STag any](model protocol.ClientModel[SJob, SProgress], environment runtime.AdapterContext, adapterSJob runtime.ValueAdapter[SJob], adapterSProgress runtime.ValueAdapter[SProgress]) (duplex.Wire, error) {
	if model == nil {
	...
	implementation, err := model(accessServer[SJob, SProgress](binding, environment, adapterSJob, adapterSProgress))
	if err != nil {
		return nil, err
	}
	if err := bindClient[SJob, SProgress](binding, func() protocol.Client[SJob, SProgress] { return implementation }, environment, adapterSJob, adapterSProgress); err != nil {
		return nil, err
	}
	complete = true
	return access, nil
...
func PrepareFromWire[SJob runtime.Of[STag], SProgress runtime.Of[STag], STag any](wire duplex.Wire, environment runtime.AdapterContext, adapterSJob runtime.ValueAdapter[SJob], adapterSProgress runtime.ValueAdapter[SProgress]) (complete func(context.Context) (protocol.ClientModel[SJob, SProgress], error), cleanup func(), err error) {
	...
	var implementation atomic.Pointer[protocol.Server[SJob, SProgress]]
	lookup := func() protocol.Server[SJob, SProgress] {
		if current := implementation.Load(); current != nil {
			return *current
		}
		return protocol.Server[SJob, SProgress]{}
	}
	if err = bindServer[SJob, SProgress](preparation.Wire(), lookup, environment, adapterSJob, adapterSProgress); err != nil {
		cleanup()
		return nil, nil, err
	}
	var checking, bound atomic.Bool
	complete = func(ctx context.Context) (protocol.ClientModel[SJob, SProgress], error) {
		...
		return func(value protocol.Server[SJob, SProgress]) (protocol.Client[SJob, SProgress], error) {
			if !bound.CompareAndSwap(false, true) {
				return protocol.Client[SJob, SProgress]{}, fmt.Errorf("model factory is already bound")
			}
			if value.Methods == nil {
				cleanup()
				return protocol.Client[SJob, SProgress]{}, fmt.Errorf("Server methods are required")
			}
			implementation.Store(&value)
			if err := preparation.Ready(); err != nil {
				cleanup()
				return protocol.Client[SJob, SProgress]{}, err
			}
			return accessClient[SJob, SProgress](preparation.Wire(), environment, adapterSJob, adapterSProgress), nil
		}, nil
	}
...
func FromWire[SJob runtime.Of[STag], SProgress runtime.Of[STag], STag any](ctx context.Context, wire duplex.Wire, environment runtime.AdapterContext, adapterSJob runtime.ValueAdapter[SJob], adapterSProgress runtime.ValueAdapter[SProgress]) (protocol.ClientModel[SJob, SProgress], error) {
...
type Recorder[SJob, SProgress any] struct {
	*duplex.RecordedWire
	events   protocol.ServerEvents[SJob, SProgress]
...
func Record[SJob runtime.Of[STag], SProgress runtime.Of[STag], STag any](ctx context.Context, target duplex.Wire, log duplex.WireLog, options duplex.RecordOptions, environment runtime.AdapterContext, adapterSJob runtime.ValueAdapter[SJob], adapterSProgress runtime.ValueAdapter[SProgress]) (*Recorder[SJob, SProgress], error) {
	...
	return &Recorder[SJob, SProgress]{RecordedWire: wire, events: accessServer[SJob, SProgress](wire, environment, adapterSJob, adapterSProgress).Events, identity: identity, options: environment.Options}, nil
}
