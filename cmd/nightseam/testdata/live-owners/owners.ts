...

{
  ...
  try {
    for(let i=0;i<7;i++){
      ...
      const raw=combinator.exportBundleUnchecked(a,bundle,worker.adapterJob());
      const got=combinator.importBundleUnchecked(b,raw,worker.adapterJob());
...
}
{
  ...
  try {
    for(const borrow of [false,true]){
      ...
      const raw=combinator.exportBundleUnchecked(exporter,{metadata:{seed:async()=>{}},run:async(n:number)=>n},worker.adapterReport());
      ...
      assert.throws(()=>combinator.importBundleUnchecked(batch,raw,worker.adapterReport()),isCode('too_many_imports'));
