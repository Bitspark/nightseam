...
export class Client implements Caller {
  ...
  async toolkit(params: Protocol.ToolkitRequest, options?: CallOptions & { owner?: LiveOwner }): Promise<Protocol.Toolkit> { const scope = scopeOf(this.peer); if (!scope) throw new DuplexError('scope_closed', 'the connection carries no live scope'); const owner = options?.owner ?? scope.owner(); if (owner.scope !== scope) throw new DuplexError('reference_foreign', 'the owner belongs to another connection'); const sent = (() => { const converted = params; validateWire({"kind":"record","fields":[{"name":"seed","type":"Count","required":true}]}, converted); return converted; })(); const result = await this.peer.call<unknown>("toolkit", sent, options); validateWire("Toolkit", result); return owner.importValue((owner) => conversion.importToolkit(owner, result)); }
}
