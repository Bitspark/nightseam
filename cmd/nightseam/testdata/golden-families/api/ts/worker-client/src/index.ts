...
export class Client implements Caller {
  ...
  async start(params: Protocol.Start, options?: CallOptions & { owner?: LiveOwner }): Promise<Protocol.Job> { const scope = scopeOf(this.peer); if (!scope) throw new DuplexError('scope_closed', 'the connection carries no live scope'); const owner = options?.owner?.scope === scope ? options.owner : scope.owner(); const result = await owner.publishValue(owner => owner.exportValue((owner) => { const converted = conversion.exportStart(owner, (params) as Protocol.Start); validateWire("Start", converted); return converted; }), sent => this.peer.call<unknown>("start", sent, options)); validateWire("Job", result); return owner.importValue((owner) => conversion.importJob(owner, result)); }
