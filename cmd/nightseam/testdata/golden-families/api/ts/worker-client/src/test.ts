...
export async function smoke(model: Protocol.ClientModel, opposite: Protocol.Server, options: Options): Promise<void> {
  ...
  if(!hasMethod(value?.methods,"supervise"))throw new Error("model method supervise is required");
  return { ...value, methods: {
  async supervise(input,context) { const key="supervise"; seen.set(key,(seen.get(key)??0)+1); const tracked=inputs.has(key),expected=inputs.get(key);inputs.delete(key); if(tracked)try { await compare("supervise.request",{ok:true,value:expected},{ok:true,value:input},options.equal); } catch(error){inputError=error;throw error;} return value.methods.supervise(input,context); },
  } }; };
  ...
  {
  let input: unknown;
  if (options.inputs && Object.prototype.hasOwnProperty.call(options.inputs,"supervise")) { input=options.inputs["supervise"]; } else {
