...
import type { AnyFamily, FamilyBinding } from "@example/supervisor-client/types";
...
export async function smoke(model: Protocol.ServerModel, opposite: Protocol.Client, options: Options): Promise<void> {
  ...
  if(!hasMethod(value?.methods,"shift"))throw new Error("model method shift is required");
  if(!hasMethod(value?.methods,"relieve"))throw new Error("model method relieve is required");
  if(!hasMethod(value?.methods,"watch"))throw new Error("model method watch is required");
  return { ...value, methods: {
  async shift(input,context) { const key="shift"; seen.set(key,(seen.get(key)??0)+1); const tracked=inputs.has(key),expected=inputs.get(key);inputs.delete(key); if(tracked)try { await compare("shift.request",{ok:true,value:expected},{ok:true,value:input},options.equal); } catch(error){inputError=error;throw error;} return value.methods.shift(input,context); },
  async relieve(input,context) { const key="relieve"; seen.set(key,(seen.get(key)??0)+1); const tracked=inputs.has(key),expected=inputs.get(key);inputs.delete(key); if(tracked)try { await compare("relieve.request",{ok:true,value:expected},{ok:true,value:input},options.equal); } catch(error){inputError=error;throw error;} return value.methods.relieve(input,context); },
  async watch(input,context) { const key="watch"; seen.set(key,(seen.get(key)??0)+1); const tracked=inputs.has(key),expected=inputs.get(key);inputs.delete(key); if(tracked)try { await compare("watch.request",{ok:true,value:expected},{ok:true,value:input},options.equal); } catch(error){inputError=error;throw error;} return value.methods.watch(input,context); },
  } }; };
  ...
  {
  let input: unknown;
  if (options.inputs && Object.prototype.hasOwnProperty.call(options.inputs,"shift")) { input=options.inputs["shift"]; } else {
  ...
  {
  let input: unknown;
  if (options.inputs && Object.prototype.hasOwnProperty.call(options.inputs,"relieve")) { input=options.inputs["relieve"]; } else {
  ...
  {
  let input: unknown;
  if (options.inputs && Object.prototype.hasOwnProperty.call(options.inputs,"watch")) { input=options.inputs["watch"]; } else {
