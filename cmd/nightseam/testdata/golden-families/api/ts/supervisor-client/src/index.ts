...
export class Client implements Caller {
  ...
  async relieve(params: Protocol.RelieveRequest, options?: CallOptions & { owner?: LiveOwner }): Promise<Protocol.Shift> { const scope = scopeOf(this.peer); if (!scope) throw new DuplexError('scope_closed', 'the connection carries no live scope'); const owner = options?.owner?.scope === scope ? options.owner : scope.owner(); const result = await owner.publishValue(owner => owner.exportValue((owner) => { const converted = conversion.exportRelieveRequest(owner, (params) as Protocol.RelieveRequest); validateWire({"kind":"record","fields":[{"name":"shift","type":"Shift","required":true},{"name":"sink","type":"worker.ProgressSink","required":true}]}, converted); return converted; }), sent => this.peer.call<unknown>("relieve", sent, options)); validateWire("Shift", result); return result as Protocol.Shift; }
  /** Takes an imported callback record. */
  async watch(params: Protocol.Watch, options?: CallOptions & { owner?: LiveOwner }): Promise<worker.Job> { const scope = scopeOf(this.peer); if (!scope) throw new DuplexError('scope_closed', 'the connection carries no live scope'); const owner = options?.owner?.scope === scope ? options.owner : scope.owner(); const result = await owner.publishValue(owner => owner.exportValue((owner) => { const converted = conversion.exportWatch(owner, (params) as Protocol.Watch); validateWire("Watch", converted); return converted; }), sent => this.peer.call<unknown>("watch", sent, options)); validateWire("worker.Job", result); return owner.importValue((owner) => live_worker.importJob(owner, result)); }
}
