...
import (
	...
	json "encoding/json"
	errors "errors"
	adapter "example.test/generated/api/go/album-binding"
...
type observingMethods[AEnvelope, BEnvelope any] struct {
	inner protocol.ServerMethods[AEnvelope, BEnvelope]
	check func(string, any) error
}

func (o observingMethods[AEnvelope, BEnvelope]) Look(ctx context.Context, params protocol.Mine[AEnvelope]) (protocol.Both[AEnvelope, BEnvelope], error) {
	if err := o.check("look", params); err != nil {
...
func Smoke[AEnvelope runtime.Of[ATag], BEnvelope runtime.Of[BTag], ATag, BTag any](ctx context.Context, model protocol.ServerModel[AEnvelope, BEnvelope], opposite protocol.Client[AEnvelope, BEnvelope], options Options, adapterAEnvelope runtime.ValueAdapter[AEnvelope], adapterBEnvelope runtime.ValueAdapter[BEnvelope]) error {
	...
	observed := func(remote protocol.Client[AEnvelope, BEnvelope]) (protocol.Server[AEnvelope, BEnvelope], error) {
		value, err := model(remote)
		if err == nil {
			if value.Methods == nil {
				return value, fmt.Errorf("model methods are required")
			}
			value.Methods = observingMethods[AEnvelope, BEnvelope]{inner: value.Methods, check: check}
		}
		return value, err
	}
	...
	_ = direct
	{
		var input protocol.Mine[AEnvelope]
		if supplied, ok := options.Inputs["look"]; ok {
			var valid bool
			input, valid = supplied.(protocol.Mine[AEnvelope])
			if !valid {
		...
		before := seen["look"]
		mutex.Unlock()
		actual, actualErr := remote.Methods.Look(ctx, input)
		mutex.Lock()
		observedError := inputError
		called := seen["look"] > before
		mutex.Unlock()
		if observedError != nil {
			return observedError
		}
		if !called {
			return fmt.Errorf("look: model was not reached: %v", actualErr)
		}
		expected, expectedErr := direct.Methods.Look(ctx, input)
		if err := compare("look", expected, actual, expectedErr, actualErr, options.Equal); err != nil {
			return err
		}
	}
	return nil
