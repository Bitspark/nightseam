...
func install[SEnvelope, SPayload any](handler Handler[SEnvelope, SPayload], options *runtime.Options) error {
	...
	handlers["hold"] = func(ctx context.Context, peer *runtime.Peer, raw json.RawMessage) (any, error) {
		...
		if err := protocol.WireSchema().Bind(map[string]any{"S.Envelope": runtime.TypeArgument[SEnvelope](), "S.Payload": runtime.TypeArgument[SPayload]()}, nil).ValidateExpressionRaw(protocol.MustTypeExpression("\"Held\""), raw); err != nil {
		...
		if err = protocol.WireSchema().Bind(map[string]any{"S.Envelope": runtime.TypeArgument[SEnvelope](), "S.Payload": runtime.TypeArgument[SPayload]()}, nil).ValidateValue(protocol.MustTypeExpression("\"S.Payload\""), result); err != nil {
			return nil, err
		}
		return result, nil
	}
