...
export async function pair<T = unknown>(model: Protocol.ClientModel<T>, options: Options, binding_t: ValueAdapter<T>): Promise<{ model: Protocol.ClientModel<T>; close(): void }> {
...
export async function smoke<T = unknown>(model: Protocol.ClientModel<T>, opposite: Protocol.Server<T>, options: Options, binding_t: ValueAdapter<T>): Promise<void> {
  const bindings = {t: binding_t};
  const inputs = new Map<string,unknown>(); const seen = new Map<string,number>(); let inputError: unknown;
  const observed: Protocol.ClientModel<T> = remote => { const value=model(remote);
