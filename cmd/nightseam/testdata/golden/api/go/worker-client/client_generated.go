...
func install(handler Handler, events Events, options *runtime.Options) error {
	...
	handlers["supervise"] = func(ctx context.Context, peer *runtime.Peer, raw json.RawMessage) (any, error) {
		...
		if err = protocol.WireSchema().ValidateValue(protocol.MustTypeExpression("\"Outcome\""), result); err != nil {
			return nil, err
		}
		return result, nil
	}
...
func (c *Client) Start(ctx context.Context, params protocol.Start) (protocol.Job, error) {
	...
	if err := protocol.WireSchema().ValidateValue(protocol.MustTypeExpression("\"Start\""), params); err != nil {
		return result, err
	}
	var raw json.RawMessage
	if err := c.Peer.Call(ctx, "start", params, &raw); err != nil {
		return result, err
	}
