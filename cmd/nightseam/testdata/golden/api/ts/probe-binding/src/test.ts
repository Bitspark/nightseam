...
import type { AnyFamily, FamilyBinding } from "@example/probe-client/types";
...
export async function smoke(model: Protocol.ServerModel, opposite: Protocol.Client, options: Options): Promise<void> {
  ...
  if(!hasMethod(value?.methods,"seen"))throw new Error("model method seen is required");
  ...
  async seen(input,context) { const key="seen"; seen.set(key,(seen.get(key)??0)+1); const tracked=inputs.has(key),expected=inputs.get(key);inputs.delete(key); if(tracked)try { await compare("seen.request",{ok:true,value:expected},{ok:true,value:input},options.equal); } catch(error){inputError=error;throw error;} return value.methods.seen(input,context); },
  } }; };
  ...
  {
  let input: unknown;
  if (options.inputs && Object.prototype.hasOwnProperty.call(options.inputs,"seen")) { input=options.inputs["seen"]; } else {
  if (false) throw new Error("input seen needs a caller-supplied native value");
  input = JSON.parse("{\"at\":\"2026-01-01T00:00:00Z\",\"status\":\"ready\",\"extra\":{},\"ratio\":0.5,\"ok\":true}");
  }
