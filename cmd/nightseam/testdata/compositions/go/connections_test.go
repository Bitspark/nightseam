...
func serveModel(t *testing.T, build func(*runtime.Peer, *scope) (bitwire.Endpoint, error)) *httptest.Server {
	t.Helper()
...
func serveWorkers(t *testing.T) *workers {
	worker := newWorker()
	server := serveModel(t, func(peer *runtime.Peer, s *scope) (bitwire.Endpoint, error) {
