...
function dependencies(directory: string): string[] {
  ...
  return [manifest.dependencies, manifest.devDependencies, manifest.peerDependencies]
    .flatMap(named => Object.keys(named ?? {}));
}

/** Every source file of a package, tests and checks among them. */
function sources(directory: string): string[] {
  const src = path.join(directory, 'src');
  return exists(src) ? readdirSync(src).filter(name => name.endsWith('.ts')).map(name => path.join(src, name)) : [];
}
...
test('the components carry no OpenTelemetry dependency, and this package is the only one that does', () => {
  ...
  const carrying = packages.filter(directory => dependencies(directory).some(name => name.startsWith('@opentelemetry/')));
...
test('this package depends on the runtime and on OpenTelemetry, and on no other component', () => {
  ...
  assert.deepEqual(Object.keys(manifest.dependencies), ['@nightseam/runtime', '@opentelemetry/api', '@opentelemetry/core']);
});
