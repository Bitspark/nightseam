...
export class Client implements Caller {
  ...
  async watch(params: Protocol.Watch, options?: CallOptions): Promise<Protocol.Subscription> { const scope = scopeOf(this.peer); if (!scope) throw new DuplexError('scope_closed', 'the connection carries no live scope'); const sent = conversion.exportWatch(scope, params as Protocol.Watch); validateWire("Watch", sent); const result = await this.peer.call<unknown>("watch", sent, options); validateWire("Subscription", result); return conversion.importSubscription(scope, result); }
