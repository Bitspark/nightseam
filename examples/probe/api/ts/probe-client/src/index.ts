...
export class Client implements Caller {
  ...
  async watch(params: Protocol.Watch, options?: CallOptions & { owner?: LiveOwner }): Promise<Protocol.Subscription> { const scope = scopeOf(this.peer); if (!scope) throw new DuplexError('scope_closed', 'the connection carries no live scope'); const owner = options?.owner ?? scope.owner(); if (owner.scope !== scope) throw new DuplexError('reference_foreign', 'the owner belongs to another connection'); const sent = owner.exportValue((owner) => { const converted = conversion.exportWatch(owner, (params) as Protocol.Watch); validateWire("Watch", converted); return converted; }); const result = await this.peer.call<unknown>("watch", sent, options); validateWire("Subscription", result); return owner.importValue((owner) => conversion.importSubscription(owner, result)); }
