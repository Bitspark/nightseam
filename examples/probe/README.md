...
Probe is a working consumer of Nightseam: one family declared in
...
With Node.js 24+, pnpm 12.4.1 and Go 1.26 installed, run this from the repository
root:
...
The runner prepares packaged source, installs an isolated consumer, starts the
Go server on an ephemeral loopback port, runs the TypeScript client, checks the
exchange and stops the server. Public dependency downloads need network access.
No external service or credentials are required. Both languages are required.
Use `--keep` to retain temporary consumers for inspection.

The commands below instead install from the registries. Use the example from a
release tag for those commands: generated code on main can require unreleased
APIs. See the [catalogue](../README.md) and the
[model and approach](../../docs/runtime/examples.md) for the distinction.
