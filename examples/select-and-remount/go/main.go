...
func main() {
	...
	selected := duplex.At(shop, []string{"cart"})
	checkout := compose(duplex.RefusingOrigin{}, duplex.DeclaredChild{Key: "basket", Wire: selected}).Bind()
	call[any](ctx, checkout, []string{"basket", "add"}, "notebook")
	original := call[[]string](ctx, shop, []string{"cart", "list"}, nil)
	remounted := call[[]string](ctx, checkout, []string{"basket", "list"}, nil)
	require(reflect.DeepEqual(original, []string{"book", "pen", "notebook"}), "add did not reach original cart")
	require(reflect.DeepEqual(original, remounted), "remount lost shared state")
	fmt.Println("original cart:", strings.Join(original, ", "))
	fmt.Println("remounted cart:", strings.Join(remounted, ", "))
}
