...
func TestOverASocket(t *testing.T) {
	...
	exported, err := dialled.Export("probe/Report", func(_ context.Context, r json.RawMessage) (json.RawMessage, error) {
		reported <- string(r)
	...
	invoke, err := served.Import(arrived, "probe/Report")
	if err != nil {
		t.Fatal(err)
	}
...
func TestTheBoundsRefuseAndLeaveNothing(t *testing.T) {
	...
	defer p.Close()
	first, err := p.A.Export("probe/Report", func(_ context.Context, r json.RawMessage) (json.RawMessage, error) {
		return r, nil
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := p.A.Export("probe/Report", func(_ context.Context, r json.RawMessage) (json.RawMessage, error) {
		return r, nil
	}); err == nil {
	...
	if _, err := p.B.Import(arrived, "probe/Report"); err != nil {
		t.Fatal(err)
	}
	second, _ := p.B.Decode(json.RawMessage(`{"binding":"deadbeefdeadbeef.9","contract":"probe/Report"}`))
	if _, err := p.B.Import(second, "probe/Report"); err == nil {
...
func TestAReferenceIsNotAConstructibleValue(t *testing.T) {
	...
	if _, err := p.B.Import(forged, "probe/Report"); err == nil {
...
func TestTheObserverIsToldAndSeesNoPayload(t *testing.T) {
	...
	exported, err := sa.Export("probe/Report", func(_ context.Context, r json.RawMessage) (json.RawMessage, error) {
		return r, nil
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := sa.Export("", nil); err == nil {
		t.Fatal("an export of no contract was admitted")
	}
	raw, _ := json.Marshal(exported)
	arrived, _ := sb.Decode(raw)
	if _, err := sb.Import(arrived, "probe/Report"); err != nil {
		t.Fatal(err)
	}
