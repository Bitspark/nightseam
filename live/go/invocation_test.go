...
func TestScopedInvocationRetainsPanicBoundary(t *testing.T) {
	...
	defer p.Close()
	reference, err := p.A.Export("probe/Panic", func(context.Context, json.RawMessage) (json.RawMessage, error) { panic("private body panic") })
	if err != nil {
		t.Fatal(err)
	}
	local, err := p.A.Import(reference, "probe/Panic")
	if err != nil {
		t.Fatal(err)
	}
	func() {
	...
	remote, err := p.B.Import(arrived, "probe/Panic")
	if err != nil {
		t.Fatal(err)
	}
